...
// Primary tab names — MUST match your Google Sheet tab names exactly (case-sensitive)
// Confirmed from live sheet screenshots: all 4 tabs use capital "Shifts"
...
// Fallback tab name variants — tried if primary tab returns 0 shifts
// Handles common naming differences without crashing
const CITY_TAB_FALLBACKS: Record<string, string[]> = {
  'Berlin':    ['Berlin shifts', 'berlin shifts', 'Berlin'],
  'Frankfurt': ['Frankfurt shifts', 'frankfurt shifts', 'Frankfurt'],
  'Stuttgart': ['Stuttgart shifts', 'stuttgart shifts', 'Stuttgart'],
  'Munich':    ['Munich shifts', 'munich shifts', 'Munich'],
}
...
export function toEuroDate(raw: string): string {
  ...
  // Handle Google Sheets date serial (days since 1899-12-30)
  if (/^\d{4,6}$/.test(s)) {
    const n = parseInt(s)
    if (n > 40000 && n < 60000) {
      const epoch = new Date(1899, 11, 30)
      epoch.setDate(epoch.getDate() + n)
      return `${String(epoch.getDate()).padStart(2,'0')}.${String(epoch.getMonth()+1).padStart(2,'0')}.${epoch.getFullYear()}`
    }
  }
  return s
}
export function toISODate(raw: string): string {
  ...
  // Handle Google Sheets date serial (integer days since 1899-12-30)
  if (/^\d{4,6}$/.test(s)) {
    const n = parseInt(s)
    if (n > 40000 && n < 60000) {   // reasonable range for modern dates
      const epoch = new Date(1899, 11, 30)
      epoch.setDate(epoch.getDate() + n)
...
// Riders sheet column layouts. Detected by checking which column contains a
// recognized city name (Berlin/Munich/Frankfurt/Stuttgart/München etc).
//
// LAYOUT A (confirmed from live sheet):
//   A=MB_No | B=Name | C=Phone | D=Email | E=Active | F=City | G=Weekly_Hours | H=Cancellations
//
// LAYOUT B (older format, City in col C):
//   A=MB_No | B=Name | C=City | D=Phone | E=Email | F=Active | G=Weekly_Hours | H=Cancellations
//
// LAYOUT C (legacy, no City column at all):
//   A=MB_No | B=Name | C=Phone | D=Email | E=Active | F=Weekly_Hours | G=Cancellations

const KNOWN_CITIES = ['Berlin','Munich','München','Frankfurt','Stuttgart']

function isCityValue(v: string): boolean {
  const s = (v || '').trim()
  return KNOWN_CITIES.some(c => c.toLowerCase() === s.toLowerCase())
}

function detectRiderLayout(r: string[]): 'A' | 'B' | 'C' {
  // Layout A: city in column F (index 5)
  if (isCityValue(r[5] || '')) return 'A'
  // Layout B: city in column C (index 2)
  if (isCityValue(r[2] || '')) return 'B'
  // Otherwise: no city column present
  return 'C'
}
...
export async function getRiders(): Promise<RiderRow[]> {
  try {
    ...
    return (res.data.values || []).map(r => {
      ...
      if (layout === 'A') {
        // MB_No | Name | Phone | Email | Active | City | Weekly_Hours | Cancellations
        return {
          nb:            (r[0] || '').trim(),
          name:          (r[1] || '').trim(),
          phone:         (r[2] || '').trim(),
          email:         (r[3] || '').trim(),
          active:        parseActive(r[4] || 'TRUE'),
          city:          normalizeCity((r[5] || '').trim()),
          weeklyHours:   parseFloat(r[6] || '0') || 0,
          cancellations: parseInt(r[7]   || '0') || 0,
        }
      } else if (layout === 'B') {
        ...
        return {
          nb:            (r[0] || '').trim(),
          name:          (r[1] || '').trim(),
          city:          normalizeCity((r[2] || '').trim()),
          phone:         (r[3] || '').trim(),
          email:         (r[4] || '').trim(),
          active:        parseActive(r[5] || 'TRUE'),
          weeklyHours:   parseFloat(r[6] || '0') || 0,
          cancellations: parseInt(r[7]   || '0') || 0,
        }
      } else {
        // No city column: MB_No | Name | Phone | Email | Active | Weekly_Hours | Cancellations
        return {
          nb:            (r[0] || '').trim(),
          name:          (r[1] || '').trim(),
          city:          '',
          ...
          email:         (r[3] || '').trim(),
          active:        parseActive(r[4] || 'TRUE'),
          weeklyHours:   parseFloat(r[5] || '0') || 0,
          cancellations: parseInt(r[6]   || '0') || 0,
        }
      }
    }).filter(r => r.nb)
  } catch (e: any) {
...
// Normalize city spelling variants to match CITY_TAB keys exactly
// (e.g. "München" → "Munich")
export function normalizeCity(raw: string): string {
  const s = (raw || '').trim()
  if (!s) return ''
  const lower = s.toLowerCase()
  if (lower === 'münchen' || lower === 'munchen' || lower === 'munich') return 'Munich'
  if (lower === 'berlin')    return 'Berlin'
  if (lower === 'frankfurt') return 'Frankfurt'
  if (lower === 'stuttgart') return 'Stuttgart'
...
export async function updateRiderInSheet(nb: string, fields: {
  ...
  if (layout === 'A') {
    ...
    if (fields.city          !== undefined) data.push({ range: `Riders!F${row}`, values: [[fields.city]] })
    ...
    // Note: fields.city is silently ignored if sheet has no City column (layout C)
  }
...
function parseShiftRow(r: string[], city: string, tabName: string, rowIndex: number): ShiftRow | null {
  ...
  return {
    ...
    hours:     parseFloat((r[5] || '0').replace(/[^\d.]/g,'')) || 0,
    capacity,
    booked,
  ...
  for (const name of namesToTry) {
    try {
      const res = await sheetsClient().spreadsheets.values.get({
        ...
        range: `'${name}'!A2:K`,  // single-quotes handle spaces in tab names
      })
      const rows = res.data.values || []
      if (rows.length === 0) continue  // empty tab — try next variant
      const shifts: ShiftRow[] = []
      rows.forEach((r, i) => {
        const shift = parseShiftRow(r as string[], city, name, i + 2)
        if (shift) shifts.push(shift)
      })
      console.log(`[readCityTab] "${name}" (${city}) → ${shifts.length} shifts`)
      // Update primary tab name if we found data under a fallback
      if (name !== tabName) {
        console.log(`[readCityTab] Note: "${city}" matched fallback "${name}" not primary "${tabName}"`)
        CITY_TAB[city] = name  // cache for future writes
      }
      return shifts
    } catch (e: any) {
      console.warn(`[readCityTab] "${name}" not found, trying next...`)
      continue
    }
  }
  console.warn(`[readCityTab] No working tab found for city "${city}". Checked: ${namesToTry.join(', ')}`)
  return []
}
...
export async function updateShiftInSheet(id: string, fields: {
  ...
  for (const [city, tabName] of Object.entries(CITY_TAB)) {
    try {
      ...
      const row  = idx + 2
      const data: { range: string; values: any[][] }[] = []
      const cap     = Number((fields.capacity !== undefined ? fields.capacity : parseInt(rows[idx][6] || '5')) || 5)
      const booked  = Number((fields.booked !== undefined ? fields.booked : parseInt(rows[idx][7] || '0')) || 0)
...
export function normalizeBookingRow(r: string[]): BookingRow {
  ...
  if (layout === 'new') {
    ...
    return {
      ...
      hours:        parseFloat((r[9]  || '0').toString().replace(/[^\d.]/g,'')) || 0,
    ...
    return {
      ...
      hours:        parseFloat((r[8]  || '0').toString().replace(/[^\d.]/g,'')) || 0,
      status:       status === 'Cancelled' ? 'Cancelled' : 'Confirmed',
