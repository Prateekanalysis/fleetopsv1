...
export default function AdminPortal() {
  ...
  function exportCSV() {
    const rows = [['Booking_ID','Rider_NB','Shift_ID','Date','Start','End','Hours','Status','Cancel_Reason','Timestamp']]
    filteredBookings.forEach(b => rows.push([b.id,b.riderNb,b.shiftId,b.date||'',b.start||'',b.end||'',String(b.hours||''),b.status,b.cancelReason||'',b.timestamp||'']))
  ...
  const filteredBookings = bookings.filter(b => {
    ...
    const matchSearch = !search || (r?.name||'').toLowerCase().includes(search.toLowerCase()) || b.riderNb.toLowerCase().includes(search.toLowerCase()) || (b.shiftId||'').toLowerCase().includes(search.toLowerCase()) || (b.date||'').includes(search)
  ...
  const topbarRight = (
    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
      {lastSync && <span style={{ fontSize:11, color:'rgba(100,116,139,0.5)' }}>Synced {lastSync.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</span>}
      ...
      <Layout navItems={navItems} activePage={page} onNav={p => setPage(p as any)}
                    ...
                    return (
                      <div key={b.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 0', borderBottom: i < 5 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontSize:13, fontWeight:500, color:'#e1e7f5', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r?.name || b.riderNb}</div>
                          <div style={{ fontSize:11, color:'rgba(100,116,139,0.65)' }}>{b.shiftId} · {b.date || ''}</div>
                        </div>
          ...
          <div className="anim-fade">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:18 }}>
              {[
                ...
                { l:'Total Hours', v:`${bookings.filter(b=>b.status==='Confirmed').reduce((a,b)=>a+(b.hours||0),0).toFixed(1)}h`, c:'#60a5fa' },
              ].map(s => (
            ...
            <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
              <div style={{ position:'relative', flex:1, minWidth:200 }}>
                ...
                <input className="input-field" style={{ paddingLeft:36 }} placeholder="Search by rider, NB, shift or date…" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
                        ...
                        <tr key={b.id} onMouseEnter={e => (e.currentTarget.style.background='rgba(59,130,246,0.04)')} onMouseLeave={e => (e.currentTarget.style.background='')} style={{ transition:'background 0.12s' }}>
                          ...
                          <TD>
                            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                              <Avatar name={r?.name || b.riderNb} size="sm" />
                              <div>
                                <div style={{ fontWeight:600, fontSize:13, color:'#e1e7f5' }}>{r?.name || b.riderNb}</div>
                          ...
                          <TD style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:12, fontWeight:700, color:'#e1e7f5', whiteSpace:'nowrap' }}>{b.date||'—'}</TD>
                          <TD style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:12, fontWeight:700, color:'#6ee7b7', whiteSpace:'nowrap' }}>{b.start||'—'}</TD>
                          <TD style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:12, fontWeight:700, color:'#6ee7b7', whiteSpace:'nowrap' }}>{b.end||'—'}</TD>
                          ...
                          <TD style={{ fontSize:11, color:'rgba(100,116,139,0.5)', whiteSpace:'nowrap', fontFamily:"'JetBrains Mono',monospace" }}>{b.timestamp||'—'}</TD>
                        </tr>
                      )
                    })}
                  </tbody>
          ...
          <div className="anim-fade">
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
              <div>
                ...
                <p className="page-sub">You assign NB numbers · synced with Google Sheets</p>
              </div>
              <button className="btn-primary" onClick={() => { setForm({}); setModal('addRider') }}><Plus size={15} /> Add Rider</button>
            </div>
            <div style={{ position:'relative', marginBottom:16 }}>
              <Search size={14} style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'rgba(100,116,139,0.6)' }} />
              <input className="input-field" style={{ paddingLeft:36 }} placeholder="Search by name or NB number…" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <div className="glass-card" style={{ overflow:'hidden' }}>
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead><tr><TH>Rider</TH><TH>NB</TH><TH>Phone</TH><TH>Email</TH><TH>Weekly Hours</TH><TH>Cancels</TH><TH>Status</TH><TH>Actions</TH></tr></thead>
                  <tbody>
        ...
        {page === 'hours' && (() => {
          ...
          bookings.forEach(b => {
            if (!map[b.riderNb]) {
              const r = riders.find(x => x.nb === b.riderNb)
              map[b.riderNb] = { nb:b.riderNb, name:r?.name||b.riderNb, confirmedHours:0, totalBookings:0, cancelledCount:0 }
            }
      ...
      {modal === 'addRider' && (
        <Modal title="Add Rider" subtitle="You assign the NB number — rider uses it to log in" onClose={() => setModal(null)}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
            <div><label style={{ display:'block', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color:'rgba(100,116,139,0.8)', marginBottom:7 }}>NB Number</label><input className="input-field" placeholder="e.g. NB1006" value={form.nb||''} onChange={e => setForm({...form,nb:e.target.value})} /></div>
