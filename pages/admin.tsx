...
// Normalizes any date string from the sheet (ISO, DD.MM.YYYY, or stray
// whitespace) to a canonical YYYY-MM-DD for safe equality comparisons.
// Without this, a single formatting drift in a sheet cell can silently
// zero out "today's shifts" or "Live Now" with no visible error.
...
export default function AdminPortal() {
  ...
  const loadAll = useCallback(async (silent=false) => {
    ...
    try {
      ...
      else failures.push(`riders (${rRes.status})`)
      if (sRes.ok) { const d=await sRes.json(); setShifts(Array.isArray(d)?d:[]) }
      else failures.push(`shifts (${sRes.status})`)
      if (bRes.ok) { const d=await bRes.json(); setBookings(Array.isArray(d)?d:[]) }
      else failures.push(`bookings (${bRes.status})`)

      if (failures.length) {
        setSyncError(`Failed to load: ${failures.join(', ')} — ${new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}`)
        if (!silent) showToast('error', `Failed to load: ${failures.join(', ')}`)
      } else {
        setSyncError(null)
      }
      setLastSync(new Date())
    } catch (e:any) {
      setSyncError('Sync failed (network error) ' + new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}))
  ...
  function getShiftStatus(s:Shift) {
    if (toISO(s.date)!==todayStr) return 'scheduled'
  ...
  const liveBookings = bookings.filter(b => {
    ...
    if (!s||toISO(s.date)!==todayStr) return false
              ...
              {cityStats.map(cs=>{
                ...
                return (
                  <div key={cs.city} className="glass-card" style={{padding:'18px 20px',border:`1px solid ${c.border}`,background:`linear-gradient(135deg,${c.bg},rgba(8,15,32,0.6))`}}>
                    ...
                    {cs.totalShifts>0&&cs.upcomingShifts===0&&<div style={{fontSize:11,color:'#fb7185',marginTop:8,display:'flex',alignItems:'center',gap:4,fontWeight:600}}><AlertTriangle size={11}/>All {cs.totalShifts} shifts are dated in the past — riders see nothing. Add upcoming shifts.</div>}
                  </div>
                )
              })}
            </div>
