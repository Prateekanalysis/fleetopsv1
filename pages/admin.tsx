...
export default function AdminPortal() {
  ...
  const cityStats = useMemo(()=>CITIES.map(city=>{
    ...
    return {
      ...
      totalHours:   cBookings.filter(b=>b.status==='Confirmed').reduce((a,b)=>a+(b.hours||0),0),
    }
              ...
              {cityStats.map(cs=>{
                ...
                return (
                  <div key={cs.city} className="glass-card" style={{padding:'18px 20px',border:`1px solid ${c.border}`,background:`linear-gradient(135deg,${c.bg},rgba(8,15,32,0.6))`}}>
                    ...
                    {cs.utilization>=90&&<div style={{fontSize:11,color:'#fcd34d',marginTop:8,display:'flex',alignItems:'center',gap:4}}><Flame size={11} color="#fcd34d"/>High demand</div>}
...
const Flame = ({size,color}:{size:number;color?:string}) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill={color||'currentColor'}><path d="M12 2c0 0-4 3-4 8s4 4 4 8c0-4 4-4 4-8S12 2 12 2z"/></svg>
)
