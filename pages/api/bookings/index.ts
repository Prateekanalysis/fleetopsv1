...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (dailyHours  + shift.hours > DAILY_LIMIT)  return res.status(400).json({ error: `Daily limit: ${+(DAILY_LIMIT-dailyHours).toFixed(1)}h left today (max ${DAILY_LIMIT}h/day)` })
      if (weeklyHours + shift.hours > WEEKLY_LIMIT) return res.status(400).json({ error: `Weekly limit: ${+(WEEKLY_LIMIT-weeklyHours).toFixed(1)}h left (max ${WEEKLY_LIMIT}h/week)` })
