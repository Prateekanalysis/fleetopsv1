...
import {
  ...
  getShifts, updateShiftInSheet, getRiders, updateRiderInSheet
} from '../../../lib/sheets'
...
// Normalise a date string to YYYY-MM-DD for internal comparison
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      // ── DAILY LIMIT ──
      const shiftDateISO = toISO(shift.date)
      const dailyShifts = myShifts.filter(s => toISO(s.date) === shiftDateISO)
      ...
      if (dailyHours + shift.hours > DAILY_HOUR_LIMIT) {
        ...
        return res.status(400).json({
          error: `Daily limit reached. You have ${remaining}h left on ${shift.date} (max ${DAILY_HOUR_LIMIT}h/day).`
        })
      }
