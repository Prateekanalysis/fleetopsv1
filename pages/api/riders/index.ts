...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    if (!requireAdmin(req, res)) return
    const { nb, name, phone, email } = req.body
    if (!nb || !name) return res.status(400).json({ error: 'NB number and name are required' })
    try {
      const existing = await getRiders()
      if (existing.find(r => r.nb.toUpperCase() === nb.toUpperCase())) {
        return res.status(400).json({ error: 'NB number already exists' })
      }
      await addRiderToSheet({ nb: nb.toUpperCase().trim(), name, phone: phone || '', email: email || '' })
  ...
  if (req.method === 'PATCH') {
    ...
    if (!nb) return res.status(400).json({ error: 'NB number required' })
    try {
