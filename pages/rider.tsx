...
export default function RiderPortal() {
  ...
  const loadData = useCallback(async (silent=false) => {
    ...
    try {
      ...
      else { console.error('shifts API error', sRes.status); if (!silent) showToast('error','Failed to load shifts. Retrying shortly.') }
      if (bRes.ok) { const d=await bRes.json(); setBookings(Array.isArray(d)?d:[]) }
      else { console.error('bookings API error', bRes.status); if (!silent) showToast('error','Failed to load bookings. Retrying shortly.') }
  ...
  // If today has zero shifts for the rider's city but a future date does,
  // jump straight to that date on first load so the rider isn't staring at
  // an empty "today" tab while shifts exist a couple of days out.
  const [autoJumped, setAutoJumped] = useState(false)
  useEffect(() => {
    if (!rider || autoJumped || shifts.length === 0) return
    const city = rider.city || selectedCity
    if (!city) return
    const todayISO = localDateISO()
    const cityShiftsNow = shifts.filter(s => !s.city || s.city.trim().toLowerCase()===city.trim().toLowerCase())
    const hasToday = cityShiftsNow.some(s => toISO(s.date) === selectedDate)
    if (hasToday) { setAutoJumped(true); return }
    const nextFuture = cityShiftsNow
      .map(s => toISO(s.date))
      .filter(d => d && d >= todayISO)
      .sort()[0]
    if (nextFuture) setSelectedDate(nextFuture)
    setAutoJumped(true)
  }, [rider, shifts, selectedCity, autoJumped, selectedDate])
  ...
  // City-filtered shifts. Both city and date use normalized comparisons so
  // formatting differences in the sheet (whitespace, separators, serials)
  // never silently produce an empty list.
  const cityShifts = activeCity
    ? shifts.filter(s => !s.city || s.city.trim().toLowerCase()===activeCity.trim().toLowerCase())
    : shifts
  const dayShifts  = cityShifts.filter(s => toISO(s.date)===selectedDate)
          ...
          <div className="anim-fade" dir={isRTL?'rtl':'ltr'}>
            ...
            <div className="date-tabs" style={{marginBottom:22}}>
              {next30.map((d,i) => {
                ...
                const hasShifts = cityShifts.some(s=>toISO(s.date)===dk)
            ...
            {!loading && dayShifts.length===0 && (() => {
              const todayISO = localDateISO()
              const cityAll = cityShifts
              const futureCityShifts = cityAll.filter(s => toISO(s.date) >= todayISO)
              const pastCityShifts   = cityAll.filter(s => toISO(s.date) <  todayISO)
              const nextFuture = futureCityShifts
                .map(s => toISO(s.date))
                .filter(Boolean)
                .sort()[0]

              if (cityAll.length === 0) {
                // Nothing at all for this city in the sheet
                return <EmptyState icon={CalendarDays} title={t('noShifts')} description={`${t('noShiftsSub')} ${activeCity} ${t('noShiftsSub2')}`}/>
              }
              if (futureCityShifts.length === 0 && pastCityShifts.length > 0) {
                // City has shifts, but every one of them is dated before today —
                // this is the exact "all shifts are stale" situation, made explicit
                // instead of looking identical to "city has zero shifts ever".
                return <EmptyState icon={CalendarDays} title={t('noShifts')}
                  description={`${activeCity} has ${pastCityShifts.length} shift${pastCityShifts.length!==1?'s':''} in the sheet, but ${pastCityShifts.length!==1?'they are':'it is'} all dated before today (${fmtDateISO(todayISO,lang)}). Ask your admin to add upcoming ${activeCity} shifts.`}/>
              }
              if (nextFuture && nextFuture !== selectedDate) {
                // City has future shifts, just not on the currently selected date —
                // point the rider directly at the next date that has them.
                return <EmptyState icon={CalendarDays} title={t('noShifts')}
                  description={`No ${activeCity} shifts on this date. The next available date is ${fmtDateISO(nextFuture,lang)}.`}/>
              }
              return <EmptyState icon={CalendarDays} title={t('noShifts')} description={`${t('noShiftsSub')} ${activeCity} ${t('noShiftsSub2')}`}/>
            })()}
